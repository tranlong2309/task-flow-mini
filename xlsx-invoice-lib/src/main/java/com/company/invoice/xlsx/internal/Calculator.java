...
public final class Calculator {
    ...
    private static List<RowError> validateItem(InvoiceItem item, int rowNumber) {
        ...
        if (item.itemName() == null || item.itemName().trim().isEmpty()) {
            ...
            errors.add(error(rowNumber, null, item.itemName(), ErrorCode.TEXT_TOO_LONG, "item_name exceeds 255 characters"));
        }
        if (item.quantity() == null) errors.add(error(rowNumber, null, "", ErrorCode.MISSING_VALUE, "quantity is required"));
        else if (item.quantity().signum() <= 0) errors.add(error(rowNumber, null, item.quantity().toPlainString(), ErrorCode.OUT_OF_RANGE, "quantity must be greater than 0"));
        else if (item.quantity().scale() > 3) errors.add(error(rowNumber, null, item.quantity().toPlainString(), ErrorCode.TOO_MANY_DECIMALS, "quantity has more than 3 decimal places"));
        if (item.unitPrice() == null) errors.add(error(rowNumber, null, "", ErrorCode.MISSING_VALUE, "unit_price is required"));
        else if (item.unitPrice().signum() < 0) errors.add(error(rowNumber, null, item.unitPrice().toPlainString(), ErrorCode.OUT_OF_RANGE, "unit_price must not be negative"));
        if (item.vatRate() == null) errors.add(error(rowNumber, null, "", ErrorCode.MISSING_VALUE, "vat_rate is required"));
        else if (item.vatRate().signum() < 0 || item.vatRate().compareTo(ONE_HUNDRED) > 0) errors.add(error(rowNumber, null, item.vatRate().toPlainString(), ErrorCode.OUT_OF_RANGE, "vat_rate must be between 0 and 100"));
        return errors;
    }
